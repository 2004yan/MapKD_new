...
import pandas as pd
from .const import CLASS2LABEL,CLASS2LABEL_BD
...
class VectorizedLocalMap(object):
    def __init__(self,
        ...
        options = ['trunk', 'primary', 'secondary', 'tertiary', 'unclassified', 'residential', # road
                ...
                'living_street',  'road',  # Special road  'service', 'service',
                ]
        ...
        for loc in self.MAPS:
            sd_map = gpd.read_file('./data_osm/osm/{}.shp'.format(loc))
    ...
    def gen_vectorized_samples(self, location, ego2global_translation, ego2global_rotation):
        ...
        line_geom = self.get_map_geom(patch_box, patch_angle, self.line_classes, location) # location是字符串，表示所在城市吧
        ...
        for line_type, vects in line_vector_dict.items():
            for line, length in vects:
                vectors.append((line.astype(float), length, CLASS2LABEL.get(line_type, -1))) # dict.get() 第二个参数为默认值，如果key不在dict中返回-1
        ...
        for contour, length in poly_bound_list:
            # vectors.append((contour.astype(float), length, CLASS2LABEL_BD.get('contours', -1)))
        ...
        # 1.测试sd_map与nuscene合并绘制结果
        # import matplotlib.pyplot as plt
        # fig, ax = self.nusc_maps[location].render_centerlines(resolution_meters=0.1, figsize=4.0)
        # for geom in self.sd_maps[location].geoms:
        #     x, y = geom.coords.xy
        #     ax.plot(x,y,'r')
        # ax.set_aspect(1)
        # ax.plot(map_pose[0], map_pose[1], 'r*', linewidth=100)
        # plt.savefig('images/3_nu_osm_{}.png'.format(location))
        # plt.close()
        # ********************************
        # patch_box_osm = (map_pose[0], map_pose[1], 200, 400)
        ...
        # if not len(osm_geom) == len(osm_vector_list):
        #     print("origin: {}, after: {}".format(len(osm_geom), len(osm_vector_list)))
        # 2. 测试提取到的patch内的向量并绘制结果
        # patch = NuScenesMapExplorer.get_patch_coord(patch_box, patch_angle)
        # import matplotlib.pyplot as plt
        # fig, ax = self.nusc_maps[location].render_centerlines(resolution_meters=0.1, figsize=4.0)
        # for geom in self.sd_maps[location].geoms:
        ...
        # ax.plot(map_pose[0], map_pose[1], 'r*', linewidth=100)
        # x, y = patch.coords.xy
        # ax.plot(x,y,'g',linewidth=5) # 绘制patch框
        # plt.savefig('images/3_nu_osm_{}.png'.format(location))
        # plt.close()
        
        # for vector in osm_vector_list:
        #     plt.plot(vector[0][:, 0], vecotr[0][:, 1],'r')
        # plt.savefig('images/sd_vector.png')

        return filtered_vectors, polygon_geom, osm_vector_list # 返回的为列表；pts为n*2的numpy矩阵，表示线的点坐标；pts_num为n；type表示标签

    def gen_vectorized_samples_only_bd(self, location, ego2global_translation, ego2global_rotation):
        map_pose = ego2global_translation[:2]
        rotation = Quaternion(ego2global_rotation)
        ...
        for ped_line, length in ped_vector_list:
            vectors.append((ped_line.astype(float), length, CLASS2LABEL_pd.get('ped_crossing', -1)))
    ...
    def get_map_geom(self, patch_box, patch_angle, layer_names, location):
        map_geom = []
        for layer_name in layer_names: # layer_names为输入欲查找的地图元素
    ...
    def poly_geoms_to_vectors(self, polygon_geom):
        ...
        lanes = polygon_geom[1][1] # lane         MultiPolygon
        ...
        for poly in union_segments.geoms:
            exteriors.append(poly.exterior) # 取外环
            for inter in poly.interiors:
                interiors.append(inter)

        results = []
        for ext in exteriors:
            if ext.is_ccw: # Returns True if a linestring or linearring is counterclockwise
                ext.coords = list(ext.coords)[::-1]
            lines = ext.intersection(local_patch) # 取外轮廓与local patch交的部分
