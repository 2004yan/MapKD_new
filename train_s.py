...
# Define distillation loss between teacher and student models
class CrossDistillationLoss(nn.Module):
    ...
    def forward(self, student_camera_bev, coach_bev, teacher_fused_bev):
        student_camera_patches = to_2d_patches(student_camera_bev)
        teacher_fused_patches = to_2d_patches(teacher_fused_bev)

        teacher_attention = compute_cross_modal_attention(teacher_fused_patches)
        student_attention = compute_cross_modal_attention(student_camera_patches)

        attention_loss1 = F.kl_div(F.log_softmax(student_attention / self.temperature, dim=-1),
                                  F.softmax(teacher_attention / self.temperature, dim=-1), reduction='batchmean')

        feature_loss1 = F.mse_loss(student_camera_bev, teacher_fused_bev)
        loss1 = attention_loss1 + feature_loss1

        student_camera_patches = to_2d_patches(student_camera_bev)
        coach_fused_patches = to_2d_patches(coach_bev)

        coach_attention = compute_cross_modal_attention(coach_fused_patches)
        attention_loss2 = F.kl_div(F.log_softmax(student_attention / self.temperature, dim=-1),
                                  F.softmax(coach_attention / self.temperature, dim=-1), reduction='batchmean')

        feature_loss2 = F.mse_loss(student_camera_bev, coach_bev)
        loss2 = attention_loss2 + feature_loss2
...
def train(args):
    ...
    for epoch in range(args.nepochs):
        for batchi, (imgs, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, 
            ...
            final_loss = seg_loss * 1.0 + var_loss * args.scale_var + dist_loss * args.scale_dist + direction_loss * args.scale_direction + 0.5 * student_bevdistill1 + 0.01 * student_bevdistill2 + 0.75 * student_segdistill1 + 0.01 * student_segdistill2
            final_loss.backward()
            ...
            if counter % 100 == 0:
                logger.info(f"TRAIN[{epoch:>3d}]: [{batchi:>4d}/{last_idx}] Time: {t1 - t0:>7.4f} Final Loss: {final_loss.item():>7.4f} IOU: {np.array2string(iou[1:].cpu().numpy(), precision=3, floatmode='fixed')}")
...
if __name__ == '__main__':
    ...
    # logging config
    parser.add_argument("--logdir", type=str, default='./output/distill_s')
    # nuScenes config
