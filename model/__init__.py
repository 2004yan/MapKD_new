...
from .pmapnet_sdmap_C import PMapNet_sdmap_C
from .pmapnet_sdmap_S import PMapNet_sdmap_S
...
from .pmapnet_mae_head_C import PMapNet_mae_head_C
def get_model(method, args, data_conf, instance_seg=True, embedded_dim=16, direction_pred=True, angle_class=36):
    ...
    elif method == 'PMapNet_sdmap_C':
        model = PMapNet_sdmap_C(data_conf, args, instance_seg=instance_seg, embedded_dim=embedded_dim, direction_pred=direction_pred, direction_dim=angle_class, sim_lidar=True)       
    elif method == 'PMapNet_sdmap_S':
        model = PMapNet_sdmap_S(data_conf, args, instance_seg=instance_seg, embedded_dim=embedded_dim, direction_pred=direction_pred, direction_dim=angle_class, lidar=False)       
    ...
    elif method == 'PMapNet_mae_head_C':
        model = PMapNet_mae_head_C(data_conf, args, instance_seg=instance_seg, embedded_dim=embedded_dim, direction_pred=direction_pred, direction_dim=angle_class, sim_lidar=True) 
    else:
