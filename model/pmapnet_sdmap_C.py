...
class PMapNet_sdmap_C(nn.Module):
    def __init__(self,  data_conf,args, instance_seg=True, embedded_dim=16, direction_pred=True, direction_dim=36, sim_lidar=True):
        super(PMapNet_sdmap_C, self).__init__()
        ...
        if sim_lidar:
            ...
            self.pp =LiftSplat(data_conf,data_aug_conf,128, True, 16)
        # else:
        ...
        self.expand_to_128 = nn.Conv2d(64, 128, kernel_size=1)
    ...
    def forward(self, img, trans, rots, intrins, post_trans, post_rots, lidar_data, lidar_mask, car_trans, yaw_pitch_roll, osm):
        # img:[1, 6, 3, 128, 352] trans:[1, 6, 3] rots:[1, 6, 3, 3] intrins:[1, 6, 3, 3] 
        # post_rots:[1, 6, 3] lidar_data:[1, 81920, 5] lidar_mask:[1, 81920]  car_trans:[1, 3] yaw_pitch_roll:[1, 3] 
        ...
        topdown_L=topdown
        if self.sim_lidar:
            sim_lidar_feature = self.pp(img, rots, trans, intrins, post_rots, post_trans, yaw_pitch_roll)
            
            sim_lidar_feature=self.expand_to_128(sim_lidar_feature)
            sim_lidar_feature=sim_lidar_feature.permute(0, 1, 3, 2)
            # sim_lidar_feature = F.interpolate(sim_lidar_feature, size=(topdown.shape[2], topdown.shape[3]), mode='bilinear', align_corners=True)

            topdown = torch.cat([topdown_L, sim_lidar_feature], dim=1)
            # import pdb;pdb.set_trace()
        ...
        return self.bevencode(bev_final),topdown_L,sim_lidar_feature,bev_small,bev_out
